#!/usr/bin/env node
// CI byte-identical reproducibility check for elixir.wasm.
...
const committed = await readFile(path.join(repoRoot, "elixir.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed elixir.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-elixir-verify-"));
...
console.log(`rebuilt elixir.wasm sha256: ${rebuiltHash}`);
