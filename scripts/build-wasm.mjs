#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-elixir.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-elixir-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/elixir-lang/tree-sitter-elixir.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as elixir.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "elixir.wasm"));
const bytes = (await readFile(path.join(repoRoot, "elixir.wasm"))).length;
console.log(`elixir.wasm: ${bytes} bytes (built from ${pin})`);
